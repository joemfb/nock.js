...
function cut (a, b, c, d) {
  ...
  if (unboxed(d)) return (d >>> low) & dec(bex(high))

  var il = (low / 26) | 0
  var ih = ((low + high) / 26) | 0

  if (il === ih) return (d.words[il] >>> (low % 26)) & dec(bex(high))

  var lowBits = 26 - (low % 26)
  var highBits = (high % 26) - lowBits
  var rem = d.words[il] >>> (low % 26)

  var ret = d.clone()
  ret.words = d.words.slice(il + 1, ih)
  ret.words.push(d.words[ih] & dec(bex(highBits)))
...
function end (a, b, c) {
  ...
  if (unboxed(c)) return c & dec(bex(d))

  if (d < 26) return c.words[0] & dec(bex(d))
  ...
  ret.words = ret.words.slice(0, i)
  ret.words.push(c.words[i] & dec(bex(d % 26)))
  ret.length = i + 1
