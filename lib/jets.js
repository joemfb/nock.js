...
function shaf (a, b) {
  var c = shas(a, b)
  return mix(end(7, 1, c), rsh(7, 1, c))
}

function sham (a) {
  if (a instanceof Cell) return shaf(atom.util.fromCord('sham'), jam(a))

  return shaf(atom.util.fromCord('mash'), a)
}

function shas (a, b) {
  return shax(mix(a, shax(b)))
}
...
function shax (a) {
  ...
  hash.update(new Buffer(atom.util.toBytes(a)))
  return atom.util.fromBytes(hash.digest())
}
